...
import { Notification, NOTIFICATION_TYPE_LABELS, NOTIFICATION_TYPE_ICONS } from '../../../core/models/notification.model';
...
@Component({
  selector: 'ab-student-notifications',
  standalone: true,
  imports: [CommonModule, LoadingSpinnerComponent, EmptyStateComponent, ErrorStateComponent, NotificationItemComponent],
  template: `
...
export class StudentNotificationsComponent implements OnInit {
  ...
  getActionRoute(notification: Notification): string {
    ...
    if (data.mentorshipRequestId) return `/student/mentorship/requests`;
    if (data.mentorshipId) return `/student/mentorship`;
    if (data.bookingId) return `/student/bookings`;
    if (data.sessionId) return `/student/sessions`;
    if (data.reviewId) return `/student/reviews`;
