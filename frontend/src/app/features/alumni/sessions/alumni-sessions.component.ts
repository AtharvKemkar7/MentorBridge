...
import { CompleteSessionDto, SessionFeedbackDto } from '../../../core/models/session.model';

@Component({
  selector: 'ab-alumni-sessions',
  standalone: true,
              ...
              <article class="session-card" [class]="'status-' + session.status.toLowerCase()">
                <div class="session-header">
                  <div class="student-info">
                    <div class="avatar" [style.background-image]="session.student?.avatarUrl ? 'url(' + session.student!.avatarUrl + ')' : ''">
                      @if (!session.student?.avatarUrl) {
                        {{ getInitials(session.student?.firstName + ' ' + session.student?.lastName) }}
                      }
                    </div>
                    <div class="student-details">
                      <span class="student-name">{{ session.student?.firstName }} {{ session.student?.lastName }}</span>
                ...
                <div class="session-footer">
                  <div class="session-actions">
                    ...
                    @if (session.status === 'IN_PROGRESS') {
                      <button type="button" class="btn-primary btn-sm" (click)="completeSession(session)">
                ...
                @if (session.status === 'COMPLETED' && alumniReviews()[session.id]) {
                  <div class="review-preview">
                    <div class="review-header">
                      <ab-rating-display [value]="alumniReviews()[session.id]!.rating" [showValue]="true" [showCount]="false" [readonly]="true" [size]="'sm'" />
                      <span class="review-date">{{ formatDate(alumniReviews()[session.id]!.createdAt) }}</span>
                    </div>
                    <p class="review-comment">{{ alumniReviews()[session.id]!.comment }}</p>
                  </div>
                }
              </article>
            }
...
export class AlumniSessionsComponent implements OnInit {
  ...
  alumniReviews = signal<Record<string, Review>>({});
  ...
  loadSessions(): void {
    ...
    this.sessionService.getAlumniSessions({
  ...
  loadAlumniReviews(): void {
    this.reviewService.getAlumniReviews({ page: 0, size: 50 }).subscribe({
      next: (res) => {
        const reviewsMap: Record<string, Review> = {};
        res.content.forEach(r => reviewsMap[r.sessionId] = r);
        this.alumniReviews.set(reviewsMap);
      },
      error: () => {}
    });
  }
  ...
  hasReview(sessionId: string): boolean {
    return !!this.alumniReviews()[sessionId];
  }

  completeSession(session: Session): void {
    const payload: CompleteSessionDto = { sessionId: session.id };
    this.sessionService.completeSession(payload).subscribe({
      next: () => {
        this.toast.success('Session Completed', 'Session marked as completed.');
        this.loadSessions();
      },
      error: (err) => this.toast.error('Failed', err.error?.message || 'Failed to complete session')
    });
  }
  ...
  openReviewDialog(session: Session): void {
    ...
    this.reviewDialogMessage.set(`Rate and review your ${session.sessionTypeName} session with ${session.student?.firstName} ${session.student?.lastName}`);
  ...
  getEmptyMessage(): string {
    switch (this.activeTab()) {
      case 'SCHEDULED': return 'Booked sessions will appear here once students book with you.';
      ...
      default: return 'Your session history will appear here once you start mentoring.';
    }
  }
}