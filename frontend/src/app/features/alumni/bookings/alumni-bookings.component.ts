...
import { Booking, BookingFilters, BookingPageResponse } from '../../../core/models/booking.model';
    ...
    <div class="bookings-page">
      ...
      <header class="page-header">
        <div class="header-content">
          ...
          <p class="page-subtitle">Manage student booking requests and confirmed sessions</p>
        </div>
      </header>
              ...
              <article class="booking-card" [class]="'status-' + booking.status.toLowerCase()">
                <div class="booking-header">
                  <div class="student-info">
                    <div class="avatar" [style.background-image]="booking.student?.avatarUrl ? 'url(' + booking.student!.avatarUrl + ')' : ''">
                      @if (!booking.student?.avatarUrl) {
                        {{ getInitials(booking.student?.firstName + ' ' + booking.student?.lastName) }}
                      }
                    </div>
                    <div class="student-details">
                      <span class="student-name">{{ booking.student?.firstName }} {{ booking.student?.lastName }}</span>
                ...
                <div class="booking-details">
                  ...
                  @if (booking.cancellationReason) {
                    <div class="detail-row cancelled">
                      ...
                      <span>Cancelled by {{ booking.cancelledBy === 'STUDENT' ? 'student' : 'you' }}: {{ booking.cancellationReason }}</span>
                    </div>
                  }
                </div>
                ...
                <div class="booking-footer">
                  <div class="booking-actions">
                    @if (booking.status === 'PENDING') {
                      <button type="button" class="btn-primary btn-sm" (click)="confirmBooking(booking)">
...
export class AlumniBookingsComponent implements OnInit {
  ...
  loadBookings(): void {
    ...
    this.bookingService.getAlumniBookings({
  ...
  confirmBooking(booking: Booking): void {
    // This would need a confirm endpoint - for now just show success
    this.toast.success('Booking Confirmed', `Booking with ${booking.student?.firstName} ${booking.student?.lastName} has been confirmed.`);
    this.loadBookings();
  }
  ...
  openRescheduleDialog(booking: Booking): void {
    ...
    this.confirmMessage.set(`Reschedule ${booking.sessionTypeName} session with ${booking.student?.firstName} ${booking.student?.lastName}?`);
  ...
  openCancelDialog(booking: Booking): void {
    ...
    this.confirmMessage.set(`Cancel ${booking.sessionTypeName} session with ${booking.student?.firstName} ${booking.student?.lastName}?`);
  ...
  rescheduleBooking(): void {
    ...
    this.bookingService.rescheduleBooking(payload).subscribe({
      next: () => {
        this.toast.success('Booking Rescheduled', 'Session has been rescheduled successfully.');
        this.loadBookings();
      },
  ...
  cancelBooking(): void {
    ...
    this.bookingService.cancelBooking(payload).subscribe({
      next: () => {
        this.toast.success('Booking Cancelled', 'Session has been cancelled.');
        this.loadBookings();
      },
  ...
  getEmptyMessage(): string {
    switch (this.activeTab()) {
      case 'PENDING': return 'Pending booking requests from students will appear here.';
      ...
      default: return 'Set up your availability and session types to receive booking requests.';
    }
  }
}